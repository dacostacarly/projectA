...
```{r}
a1 <- lm(Health ~ Uninsured, data = chr_2024)

model_parameters(a1)

estimate_expectation(a1, data = "grid", ci = 0.95)

ggplot(chr_2024, aes(x = Health, y = Uninsured)) +
    geom_point(alpha = 0.5) + 
    geom_smooth(method = "lm", col = "red",
                formula = y ~ x, se = TRUE) +
    geom_smooth(method = "loess", col = "blue",
                formula = y ~ x, se = FALSE) +
    labs(x = "Rate of Poor/Fair Health Outcomes (% by county)",
         y = "Rate of Uninsured (% by county)",
         title = "Do higher uninsured rates result in poorer health outcomes by county?")
